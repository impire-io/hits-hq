...
**Trigger:** any piece of work is about to start in a code repo or in hits-hq — rooted in a design or issue, or not.
...
| Work | ID |
|---|---|
| Rooted in a hits-hq design or issue | that record's number and slug (`012-oplog-compaction`) |
...
A slug is a first-class ID, not a fallback. It is still the branch name everywhere, still the workspace directory, still the PR label, and one org-wide PR query still recovers the whole piece of work. The number buys a link to a record; where no record exists yet, there is nothing to link to and nothing is lost.
...
1. **Check what is already in flight.** Before anything else:
   ...
   For issue-rooted work, also check the report's `claimed-by:` (visible in `05-TOOLS/status.sh`) — a claim without a PR yet is still someone's declared intent; talk to them before taking it, or `05-TOOLS/claim.sh <NNN> --steal` if it is truly abandoned.

   **When the work is rooted in an existing record, confirm that record is on `origin/main`** — a PR query cannot show a number claimed by work that already merged:

   ```
   git -C ~/Impire/hits-hq fetch origin
   git -C ~/Impire/hits-hq ls-tree --name-only origin/main 03-DECISIONS/ 04-ISSUES/
   ```
...
2. **Open the workspace.** One command, naming the work id and the repos the work touches:
   ...
   **hits-hq is always included, named or not** — it holds the work item, the `lands:` block, and the edit that closes the record. **Name only the code repos the work actually touches**; a repo you do not need gets no worktree.
...
3. **Declare the landing order — only if the work spans repos.** Add a `lands:` block to the hits-hq work item's frontmatter, `pr:` empty until each PR exists:
   ...
   **The last entry is always hits-hq, and it is the one that closes the record.** hits-hq appears twice in a cross-repo block, doing two different jobs:
   ...
   lands:
     ...
     - { repo: hits-api,       pr: "", after: [hits-projector] }
     - { repo: hits-mcp,       pr: "", after: [hits-api] }
     - { repo: hits-hq,        pr: "", after: [hits-mcp], closes: true }
   ```

   Where the work is rooted in a design amendment, hits-hq also leads the block (`after: []`) — the amendment is what the code is built against. The `closes: true` entry is a separate, later PR: it sets `status: resolved` and fills in `fixed-by:`.

   It must land **after** every code PR, and the constraint is mechanical rather than stylistic. A `fixed-by:` reference has to be an ancestor of that repo's `origin/main` to be verifiable ([playbook 03](03-issues.md), "Recording the refs"); a commit cannot cite itself, and a claim written before its fix merges is one `05-TOOLS/check-claims.sh` rejects.

   Without that entry the closing step belongs to nobody. hits-hq's only PR merged days before the work finished, its worktree is stale or already torn down, and recording completion becomes a job someone has to decide to start from nothing. That is where an HQ repo measurably rots: in the parent project, four issues were found fixed-but-still-open on a single afternoon, each by hand. `05-TOOLS/check-unclaimed.sh` fails CI on the strong form of it, but a check that catches the omission is a poorer instrument than a step that prevents it.

   Single-repo work gets no block — the PR is its own record, and carries the fix and the closing edit together.
...
4. **Push the first commit and open the draft PR.** Do this as early as there is anything to push; the draft PR is the claim, not the finish line.
   ...
   gh pr create --draft \
     ...
     --label "work/<work-id>" \
     --body "hits-hq: <path/to/work-item.md>
   Blocked by: <predecessor PR url, or none>"
   ```

   (Create the label first if it does not exist yet: `gh label create "work/<work-id>"`.) Fill the PR number back into the work item's `lands:` block.
...
- Do not leave a cross-repo `lands:` block without a trailing `closes: true` hits-hq entry. Work whose last act is a code merge closes nothing in hits-hq, and no one comes back for it.
