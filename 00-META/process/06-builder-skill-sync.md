...
This playbook fires reliably from hits-hq events — [playbook 04](04-build-handoff.md) step 9 runs it on every status flip. It does **not** observe an SDK release that happens without a hits-hq design change. Closing that gap needs a line in the owning code repo's `AGENTS.md`, pointing here when the wire contract changes.
